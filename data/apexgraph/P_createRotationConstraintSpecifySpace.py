@subgraph
@namespace("custom")
def P_createRotationConstraint(graph:ApexGraphHandle,nodename:String,targetname:String,weight:Float,connect:Bool=False,parentSpace = "none"):
    ...
    if parentSpace == "none":
        graph,targetrestname = custom.P_createRest(graph,targetname,targetname+"_rest4rotconstarget",mode = "world")
    if parentSpace != "none":
        graph,targetrestname = custom.P_createRest(graph,targetname,targetname+"_rest4rotconstarget",mode = "specify",specifyParentName = parentSpace)
